...
- **インラインコメントの方針**: WHAT ではなく WHY を書く。以下の判断基準に従う
  - 削除テスト: そのコメントを消して実際に読み手が困るかを問う。困らなければ書かない
  - 書くべき: コードから復元できない情報（隠れた制約、過去の不具合の回避策、直感に反する挙動）
  - 書かない: コードを読めば分かる処理説明（WHAT）、変更履歴（「〜を追加」等）、タスク/issue番号への参照
  - 変更の経緯や背景はコミットメッセージ・PR説明に書き、コード本文には残さない
