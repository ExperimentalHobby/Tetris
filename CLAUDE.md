...
- **承認されたプランは `docs/{機能名}-Plan.md` として書き出しておく**（`docs/*-Plan.md` は `.gitignore` 対象のローカル資料であり PR には含まれない。実装の経緯を後から追えるようにする）。一方 `docs/plans/` 配下の設計ドキュメントのように、実装済み機能の経緯として後から参照する価値があるものはリポジトリで共有管理する。
